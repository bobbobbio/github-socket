...
impl Client {
    fn new() -> Self {
        ...
        let token_decoded = base64::engine::general_purpose::STANDARD.decode(token_parts.next().unwrap()).unwrap();
        let v = serde_json::from_slice::<serde_json::Value>(&token_decoded).unwrap();
        println!("{v:#?}");
